# Hi, I'm Bhushan 👋

Senior Product Manager specialising in **Payments & iGaming** — 9 years of building systems that move money reliably, reduce fraud, and grow revenue.

My work sits at the intersection of player trust, checkout conversion, and payment infrastructure. I don't just ship features — I measure outcomes.

**What I work on**

- Global payment infrastructure: checkout, disbursements, PSP routing
- Fraud detection & bonus abuse systems
- A/B experimentation on conversion flows
- APM integrations & compliance (UKGC / EU)
- AI-native PM workflows (Claude, ChatGPT, Figma Make)

---

## Impact, by the numbers

| Metric | Result |
|---|---|
| Incremental quarterly revenue | **$29.6M** |
| Bonus fraud reduction | **95%** |
| Payout processing time | **72h → 8h** |
| CS ticket reduction | **42%** |
| Digital wallet registrations | **400%+ growth** |

---

## What I'm Building

### 📬 [AI Pulse for PMs](https://github.com/Bhushan7/ai-pulse-for-pms) · [🚧 In Development →](https://ai-pulse-for-pms.vercel.app)
A weekly newsletter and platform for Product Managers who want to actually understand AI — not just hear about it. Curated AI news filtered for relevance to product work, with concrete PM use cases and deep-dive links. Built with TypeScript, Supabase, and Vercel.

### 💊 [GLP-1 Companion](https://github.com/Bhushan7/glp1-companion) · [🚧 In Development →](https://myglp1companion.com)
AI-powered health tracking for people on GLP-1 medications (Ozempic, Wegovy, Mounjaro). Delivers personalised weekly AI insights correlating food, hydration, energy, and weight data — and generates doctor-visit PDF summaries. Built with Next.js 14, Supabase, Claude API, and Vercel.

---

## Career

**Senior PM — Payments & Fraud** · Patrianna *(2024 – Present)*
Checkout optimisation, payout automation, fraud detection across US social gaming brands.

**PM — Payments & Compliance** · Entain *(2022 – 2024)*
APM launches (Aircash, DIAS, BVNK Crypto), intelligent PSP routing, UKGC/EU compliance.

**PM — Digital Wallets & Partnerships** · ICICI Bank *(2021 – 2022)*
Pockets wallet roadmap, BBPS, wallet-as-a-service B2B partnerships.

**Business Analyst** · Kellton Tech *(2019 – 2020)*
FinTech & eCommerce product solutioning, ₹50M+ new business contribution.

**Software Engineer** · Infosys *(2015 – 2017)*
Enterprise payment systems for global banking clients.

---

## Tools & Stack

`JIRA` `Confluence` `Figma` `SQL` `Looker` `Tableau` `Optimizely` `SEON` `Retool` `Claude` `ChatGPT` `Vercel`

---

## GitHub Stats

![Bhushan's GitHub Stats](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Bhushan7&theme=default)

---

## Connect

Open to senior PM opportunities in **payments, fintech, or iGaming**.

- 🌐 [Portfolio](https://bhushan-murari-portfolio.vercel.app/)
- 💼 [LinkedIn](https://linkedin.com/in/bhushanmurari)
- 📧 bhushan93.m@gmail.com
